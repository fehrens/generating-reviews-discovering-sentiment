...
def train_with_reg_cv(trX, trY, vaX, vaY, teX=None, teY=None, penalty='l1',
    ...
    model.fit(trX, trY)
    #fe save the model 
    filename = pkg_resources.resource_filename('sentiment', 'model/log_reg.sav')
...
def predict_with_reg_cv(X):
    log_reg_file = Path(Path(pkg_resources.resource_filename('sentiment', 'model/log_reg.sav')))
